...
C++20では、特定のモジュールに属していない宣言はグローバルモジュールに属している。

* グローバルモジュールは名前を持たず、インポートすることはできない。
* グローバルモジュールから宣言をエクスポートすることはできない。

### モジュールリンケージ

C++20では、新たにモジュールリンケージが追加された。

* グローバルモジュール以外のモジュールに属していてエクスポートしていない名前は、モジュールリンケージを持ち同一モジュール内で参照できる。
...
複数の定義が同時に見えるようにならない限りは、ODR違反とならない。
...
* ヘッダーファイルをプリプロセスし、その時定義されていたマクロをimport宣言の直後で再定義する。
* ヘッダーファイルをフェーズ7までコンパイルし、その翻訳単位(ヘッダーユニット)をインポートする。
    * 内部リンケージを持たない名前は暗黙的にエクスポートされる。
    * 宣言はグローバルモジュールに属する。ヘッダーユニットはモジュール宣言を持てない。
